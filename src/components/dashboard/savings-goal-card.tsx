...
import { savingsGoal, usd } from "@/lib/mock-data";

export function SavingsGoalCard({ saved, target }: { saved: number; target: number }) {
  const goalPct = Math.min(100, Math.round((saved / target) * 100));
  return (
    <div className="relative flex flex-col justify-between gap-[18px] overflow-hidden rounded-2xl p-5 text-white" style={{ background: "linear-gradient(135deg,#1f9e4a 0%,#127a39 48%,#0a5a2a 100%)" }}>
      ...
      <div className="relative">
        ...
        <h3 className="text-[20px] font-bold tracking-tight">{savingsGoal.title}</h3>
      </div>
      <div className="relative">
