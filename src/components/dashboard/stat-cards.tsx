...
import { balance, dailySpend, monthlySavings, usd, pct } from "@/lib/mock-data";
...
export function StatCards({ onAddTransaction }: { onAddTransaction?: () => void }) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      <Card className="rounded-2xl border-db-line bg-db-card p-[18px]" style={{ boxShadow: "var(--db-shadow)" }}>
        ...
        <Money value={balance.total} className="mt-3 block text-[28px] font-bold tracking-tight text-db-text" />
        <div className="mt-1.5 flex items-center gap-2 text-[13px]">
          <span className="font-bold text-[#16A34A] dark:text-[#4ade80]">{pct(balance.changePct)}</span>
      ...
      <Card className="flex min-h-[186px] flex-col rounded-2xl border-db-line bg-db-card p-[18px]" style={{ boxShadow: "var(--db-shadow)" }}>
        ...
        <Money value={dailySpend.amount} className="mt-3 text-[26px] font-bold tracking-tight text-db-text" />
        <p className="mt-3 text-[13px]"><span className="font-bold text-[#16A34A] dark:text-[#4ade80]">{pct(dailySpend.changePct)}</span> <span className="text-db-soft">vs yesterday avg</span></p>
      </Card>
      ...
      <Card className="flex min-h-[186px] flex-col rounded-2xl border-db-line bg-db-card p-[18px]" style={{ boxShadow: "var(--db-shadow)" }}>
        ...
        <Money value={monthlySavings.amount} className="mt-3 text-[26px] font-bold tracking-tight text-db-text" />
        <p className="mt-3 text-[13px]"><span className="font-bold text-[#16A34A] dark:text-[#4ade80]">{pct(monthlySavings.changePct)}</span> <span className="text-db-soft">income − expenses</span></p>
      </Card>
    </div>
  );
}
